...
$academic_year = $acad_data['id'];

if (isset($_POST['id'])) {
  $_SESSION['eval_id'] = $_POST['id'];
}
$id = $_SESSION['eval_id'];
            ...
            <div class="email-app">
              <div class="email-side-nav remain-height ov-h">
                <div class="h-100 layers">
                  <div class="p-20 bgc-grey-100 layer w-100">
                    <a href="departments.php" class="btn btn-danger bdrs-50p p-15 lh-0">
                      ...
                      while ($subject_row = mysqli_fetch_assoc($subjects_query)) {
                        echo '
                              <li class="nav-item" title="' . $subject_row['description'] . '">
                                <a href="subject_evaluations.php?subject_id=' . $subject_row['subject'] . '" class="nav-link c-grey-800 cH-blue-500 actived">
              ...
              <div class="email-wrapper row remain-height bgc-white ov-h">
                ...
                <div class="email-list h-100 layers">
                  <div class="layer w-100">
                    <div class="bgc-grey-100 peers ai-c jc-sb p-20 fxw-nw">
                      ...
                      <div>
                        ...
                        <button type="submit" onclick="window.printContent()" class="btn btn-primary btn-color">
                  ...
                  <div class="layer w-100 fxg-1 scrollable pos-r" id="printable-content">


                    <?php
                    if (1==1) {
                      $comment_query = mysqli_query($conn, "SELECT *, student_evaluation.added AS comment_date FROM student_evaluation LEFT JOIN step_users ON step_users.id = student_evaluation.student_id WHERE teacher_id = '$id' AND acad_year = '$academic_year' GROUP BY student_evaluation.comment ");
                      if (mysqli_num_rows($comment_query) != 0) {
                        ...
                        while ($comment_row = mysqli_fetch_assoc($comment_query)) {
                          ...
                          $student_id = $comment_row['student_id'];
                          $get_total_score_query = mysqli_query($conn, "SELECT SUM(score) AS total_score FROM student_evaluation WHERE student_id = '$student_id' AND subject_id='0' AND acad_year='$academic_year'");
                          ...
                          $total_rating_comment = round(($total_score_comment / $total_questions), 4);

                          $rating_comment = '';
                          $badge_color_comment = '';
                          if ($total_rating_comment >= 1.00 && $total_rating_comment <= 1.75) {
                            $rating_comment = 'Unsatisfactory';
                            $badge_color_comment = 'bg-danger';
                          } else if ($total_rating_comment >= 1.76 && $total_rating_comment <= 2.50) {
                            $rating_comment = 'Satisfactory';
                            $badge_color_comment = 'bg-warning';
                          } else if ($total_rating_comment >= 2.51 && $total_rating_comment <= 3.25) {
                            $rating_comment = 'Very Satisfactory';
                            $badge_color_comment = 'bg-success';
                          } else if ($total_rating_comment >= 3.26 && $total_rating_comment <= 4.00) {
                              ...
                              <div class="">
                                ...
                                <div class="email-list-item peers fxw-nw p-20 bdB bgcH-grey-100 cur-p">
                                  <div class="peer peer-greed ov-h">
                                    ...
                                    <span class="">' . $comment_row['comment'] . '</span>
                                  </div>
                                </div>
                              </div>
                            ';
                      ...
                      <div class="bdT pX-40 pY-30">
                        ...
                        <div class="row">
                          ...
                          if (1==1) {
                            ...
                            while ($cat_rows = mysqli_fetch_assoc($categories)) {
                              ...
                              while ($row_question = mysqli_fetch_assoc($questions)) {
                                ...
                                echo '
                                        ...
                                        ';
                                $rating_query = mysqli_query($conn, "SELECT SUM(score) AS total_score FROM student_evaluation WHERE question_id = '$q_id' AND subject_id='0'  AND acad_year = '$academic_year'  AND teacher_id = '$id'");
                                ...
                                $total_evaluators_query = mysqli_query($conn, "SELECT COUNT(*) FROM student_evaluation WHERE question_id = '$q_id' AND subject_id='0'  AND acad_year = '$academic_year'  AND teacher_id = '$id' GROUP BY student_id");
                                ...
                                $rating = '';
                                $badge_color = '';
                                if ($total_rating >= 1.00 && $total_rating <= 1.75) {
                                  $rating = 'Unsatisfactory';
                                  $badge_color = 'bg-danger';
                                } else if ($total_rating >= 1.76 && $total_rating <= 2.50) {
                                  $rating = 'Satisfactory';
                                  $badge_color = 'bg-warning';
                                } else if ($total_rating >= 2.51 && $total_rating <= 3.25) {
                                  $rating = 'Very Satisfactory';
                                  $badge_color = 'bg-success';
                                } else if ($total_rating >= 3.26 && $total_rating <= 4.00) {
                                  $rating = 'Outstanding';
                                  $badge_color = 'bg-info';
                                }
                              ...
                              $sql_get_total_question_per_category = mysqli_query($conn, "SELECT COUNT(*) AS total_question FROM questionaires WHERE cat_id = '$cat_id'");
                              ...
                              $percentage = round(($tot_score / $total_questions_cat) / $total_evaluators, 2);
                              $rating_pie = '';
                              $badge_color = '';

                              if ($percentage >= 1.00 && $percentage <= 1.75) {
                                $rating_pie = 'Unsatisfactory';
                              } else if ($percentage >= 1.76 && $percentage <= 2.50) {
                                $rating_pie = 'Satisfactory';
                              } else if ($percentage >= 2.51 && $percentage <= 3.25) {
                                $rating_pie = 'Very Satisfactory';
                              } else if ($percentage >= 3.26 && $percentage <= 4.00) {
                            ...
                            echo "<center><h6>No evaluation data found.</h6></center>";
                          }
...
<script>
    function printContent() {
        // Print only the content inside the #printable-content div
